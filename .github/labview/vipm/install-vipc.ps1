...
$PublicRepoUrl    = if ($Env:VIPM_PUBLIC_REPO_URL) { $Env:VIPM_PUBLIC_REPO_URL } else { 'https://github.com/Michal-M-Kozak/Oauth2.git' }
...
function Save-PublicVipmPackage($Package, [string] $OutDir) {
    ...
    Invoke-WebRequest -Uri $url -OutFile $outFile -UseBasicParsing -MaximumRedirection 10 -TimeoutSec 300 -Headers @{ 'User-Agent' = 'Oauth2 VIPM downloader' } | Out-Null
